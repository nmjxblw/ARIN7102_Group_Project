...
# tokenizer = load_tokenizer_compat(INITIAL_BERT_MODEL_PATH, local_files_only=True)
...
class BERTManager(metaclass=SingletonMeta):
    ...
    def __init__(self, *, debug_mode=False) -> None:
        ...
        # """ BERT 模型实例，初始为 None，首次调用 predict 时加载并缓存模型以加快后续推理速度 """
        # self._tokenizer: Optional[DistilBertTokenizer] = None
        # """ BERT 分词器实例，初始为 None，首次调用 predict 时加载并缓存分词器以加快后续推理速度 """
        # self._mlb_disease: Optional[MultiLabelBinarizer] = None
        # """ 疾病标签二值化器实例，初始为 None，首次调用 predict 时加载并缓存以加快后续推理速度 """
        # self._mlb_symptom: Optional[MultiLabelBinarizer] = None
        # """ 症状标签二值化器实例，初始为 None，首次调用 predict 时加载并缓存以加快后续推理速度 """
