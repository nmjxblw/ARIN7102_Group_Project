"""
drug_embedding_engine.py  —— Member C 核心模块
使用 PubMedBERT 对药物的"语义文本"(drug_name + symptoms + disease_desc)
进行 768 维 Embedding，供后续 Faiss KNN 检索与传统 ML 重排使用。

用法:
    # 在 Notebook Block 7 中
    from drug_embedding_engine import DrugEmbeddingEngine
    engine = DrugEmbeddingEngine()
    drug_embeddings = engine.encode(df['semantic_text'].tolist())
    engine.save(drug_embeddings, 'drug_comprehensive_embeddings.npy')
"""

import os
import numpy as np
import pandas as pd
import torch  # pyright: ignore[reportMissingImports]
from transformers import AutoModel, AutoTokenizer  # pyright: ignore[reportMissingImports]
from tqdm import tqdm  # pyright: ignore[reportMissingModuleSource]


# ============================================================
# 默认配置
# ============================================================
DEFAULT_MODEL_NAME = "microsoft/BiomedNLP-PubMedBERT-base-uncased-abstract"
DEFAULT_BATCH_SIZE = 32
DEFAULT_MAX_LENGTH = 256  # 语义文本较长，给足 token 空间
DEFAULT_OUTPUT_DIR = os.path.dirname(os.path.abspath(__file__))


class DrugEmbeddingEngine:
    """
    药物语义特征向量化引擎。

    职责:
    1. 加载 PubMedBERT 医疗大模型
    2. 将 semantic_text (药名+症状+疾病描述组装文本) 批量编码为 768 维向量
    3. 保存/加载 numpy 矩阵文件
    """

    def __init__(
        self,
        model_name: str = DEFAULT_MODEL_NAME,
        device: str | None = None,
        batch_size: int = DEFAULT_BATCH_SIZE,
        max_length: int = DEFAULT_MAX_LENGTH,
    ):
        self.model_name = model_name
        self.batch_size = batch_size
        self.max_length = max_length

        # 自动选择设备: GPU > MPS(Apple Silicon) > CPU
        if device is None:
            if torch.cuda.is_available():
                self.device = "cuda"
            elif hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
                self.device = "mps"
            else:
                self.device = "cpu"
        else:
            self.device = device

        print(f"[DrugEmbeddingEngine] 设备: {self.device}")
        print(f"[DrugEmbeddingEngine] 正在加载模型: {self.model_name} ...")
        self.tokenizer = AutoTokenizer.from_pretrained(self.model_name)
        self.model = AutoModel.from_pretrained(self.model_name).to(self.device)
        self.model.eval()
        print(f"[DrugEmbeddingEngine] 模型加载完成 ")

    # ----------------------------------------------------------
    # 核心编码方法
    # ----------------------------------------------------------
    def encode(self, texts: list[str]) -> np.ndarray:
        """
        将文本列表编码为 [CLS] token 的 768 维向量。

        Args:
            texts: 每条为一款药的 semantic_text

        Returns:
            np.ndarray, shape = (len(texts), 768)
        """
        all_embeddings: list[np.ndarray] = []

        with torch.no_grad():
            for i in tqdm(range(0, len(texts), self.batch_size), desc="Encoding drugs"):
                batch = texts[i : i + self.batch_size]
                # 处理空值/NaN 占位
                batch = [
                    str(t).strip() if pd.notna(t) and str(t).strip() else "[MASK]"
                    for t in batch
                ]

                encoded = self.tokenizer(
                    batch,
                    padding=True,
                    truncation=True,
                    max_length=self.max_length,
                    return_tensors="pt",
                )
                encoded = {k: v.to(self.device) for k, v in encoded.items()}

                outputs = self.model(**encoded)
                # 取 [CLS] token 的隐藏状态作为句子级嵌入
                cls_vectors = outputs.last_hidden_state[:, 0, :].cpu().numpy()
                all_embeddings.append(cls_vectors)

        embeddings = np.vstack(all_embeddings)
        print(f"[DrugEmbeddingEngine] 编码完成: shape = {embeddings.shape}")
        return embeddings

    # ----------------------------------------------------------
    # 查询向量编码 (供后续 KNN 检索使用)
    # ----------------------------------------------------------
    def encode_query(self, query_text: str) -> np.ndarray:
        """
        将单条查询文本 (如 "symptoms: fever, headache; disease: common_cold")
        编码为 768 维向量，用于在 Faiss 中做 KNN 检索。

        Returns:
            np.ndarray, shape = (1, 768)
        """
        return self.encode([query_text])

    # ----------------------------------------------------------
    # 保存 / 加载
    # ----------------------------------------------------------
    @staticmethod
    def save(embeddings: np.ndarray, filename: str, output_dir: str = DEFAULT_OUTPUT_DIR):
        path = os.path.join(output_dir, filename)
        np.save(path, embeddings)
        print(f"[DrugEmbeddingEngine] 已保存: {path}  (shape={embeddings.shape})")
        return path

    @staticmethod
    def load(filename: str, output_dir: str = DEFAULT_OUTPUT_DIR) -> np.ndarray:
        path = os.path.join(output_dir, filename)
        embeddings = np.load(path)
        print(f"[DrugEmbeddingEngine] 已加载: {path}  (shape={embeddings.shape})")
        return embeddings
